...
def generate_scatter_plots(df: pd.DataFrame, output_dir: Path) -> str:
    """
    Gera múltiplos scatter plots das principais features.
    Similar a scatter_plots.png dos notebooks.
    ...
    print("\n  Gerando scatter plots de features...")

    numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()

    # Selecionar top 6 features (excluindo dummies)
    features = [c for c in numeric_cols if not any(p in c.lower() for p in ['cod_', 'equipamento_', 'produto_'])][:6]
    ...
    n_plots = min(6, n_features * (n_features - 1) // 2)
    ...
    for i in range(len(features)):
        for j in range(i + 1, len(features)):
            ...
            axes[plot_idx].set_xlabel(features[i][:20], fontsize=9)
            axes[plot_idx].set_ylabel(features[j][:20], fontsize=9)
    ...
    plt.suptitle("Scatter Plots das Principais Features", fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def main() -> dict:
    ...
    print("\n[5/6] Matriz de urgência...")
    path = generate_urgency_matrix(df, output_dir)
    if path:
        files_generated.append(path)

    print("\n[6/6] Scatter plots de features...")
